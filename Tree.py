...
class BinarySearchTree(TreeNode):
    ...
    def remove(self, currentNode):
        ...
        # currentNode has left or right children
        else:
            if currentNode.hasLeftChild():
                if currentNode.isLeftChild():
                    ...
                    currentNode.replaceNodeData(
                        currentNode.leftChild.key, currentNode.leftChild.payload.currentNode.leftChild.leftChild, currentNode.leftChild.rightChild)
...
# Check if a given tree is a BST with tree traversals
def validate_BST(tree):

    tree_vals = []
    if tree != None:
        validate_BST(tree.getLeftChild())
        tree_vals.append(tree.getRootVal())
        validate_BST(tree.getRightChild())

    return tree_vals == sorted(tree_vals)


# Check if a given tree is a BST with recursions
class Node():
...
def validate_BST_recursion(node, min=float('-inf'), max=float('inf')):
    if not node:
        return True
    if (node.value > min and
        node.value < max and
        validate_BST_recursion(node.left, min, node.value) and
            validate_BST_recursion(node.right, node.value, max)):
        return True
    else:
        return False
